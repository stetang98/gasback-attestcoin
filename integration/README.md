...
After a completed run, judges can run `node integration/verify-public.cjs` with **no wallet file**. It independently fetches both-chain receipts, verifies the source proof through the native precompile, matches transaction identity, checks the actual target rebate event/state and beneficiary balance change against the public manifest, and repeats the read-only duplicate rejection check.
